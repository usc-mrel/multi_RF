% Figure 4 - different field strengths simulation
...
cd(fullfile(root_path, '/multi_RF/figures/sim_results/Fig4_field_strength/'));

%% load exciatation patterns for reVERSE method
reverse_girf = zeros(80,80,3,5);
tmp = dir('./bloch_B0*.mat');
for i = 1:length(tmp)
    load(tmp(i).name);
