% Figure 3 - off-isocenter distances
...
cd(fullfile(root_path, '/multi_RF/figures/sim_results/Fig3_distance/'));

%% load exciatation patterns for reVERSE method
reverse_girf = zeros(80,80,3,7);
tmp = dir('./bloch_B00.55_offc*.mat');
for i = 1:length(tmp)
    load(tmp(i).name);
...
tmp = dir('./blochmex_B00.55_offc*.mat');
for i = 1:length(tmp)
    load(tmp(i).name);
