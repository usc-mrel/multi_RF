% Supporting information Figure S2 - duration impacts
% re-verse method vs. proposed method
% Ziwei 02192024
...
cd(fullfile(root_path, '/multi_RF/figures/sim_results/FigS2_duration/'));

%% load exciatation patterns for reVERSE method
reverse_girf = zeros(80,80,3,4);
index = [1 3 2 4];
tmp = dir('./bloch_*dr0.25cm*.mat');
for i = 1:length(tmp)
...
tmp = dir('./blochmex_*dr0.25cm*.mat');
for i = 1:length(tmp)
    load(tmp(index(i)).name);
